...
if dataGenerate == 0
    ...
    PSRF_For = 0; % 1: PSRF_For; 0: parallel
    ...
    if PSRF_For == 1
        if jj == 2
           % 2 groups (for): [1,2,3]; [1,2,5]; [1,2,3,5]
           chain_select = [1,2,3,5];
           lgd = 1;% legend or not
%            color_order = rand(30,3);
        ...
        if jj == 2
            % 2 groups (parallel): [1,2,3] 
           chain_select = [1,2,3];
           lgd = 1;% legend or not
%            color_order = rand(30,3);
        elseif jj == 3
            % 3 groups (parallel): [1,4,5]
            chain_select = [1,4,5];
            lgd = 0;% legend or not
        elseif jj == 4
            % 4 groups (for): [2,3]; 
    ...
    if PSRF_For == 1
        folder = ['results/SimulationStudy/',label,'/seed_',num2str(seed),'/PSRF_For/',... % /PSRF_For
    ...
    line([burnin,iterSize],[1,1],'linestyle','-','LineWidth',lineSZ+1,'Color',[0 0 0]');
    if jj > 1 % == 2  or > 1 (all)
    ...
    figName = ['$J=',num2str(jj),'$'];
    title(figName,'Interpreter','latex','FontSize',fontSZ,'fontname','times')
